...
ETag=$(curl --head $HostName/Content/Raef Tech-MacOS-arm64.zip | grep -i "etag" | cut -d' ' -f 2)
...
if [ -z "$UpdatePackagePath" ]; then
    ...
    curl $HostName/Content/Raef Tech-MacOS-arm64.zip --output $InstallDir/Raef Tech-MacOS-arm64.zip
else
    echo  "Copying install files..." >> /tmp/Raef Tech_Install.log
    cp "$UpdatePackagePath" $InstallDir/Raef Tech-MacOS-arm64.zip
    rm -f "$UpdatePackagePath"
fi

unzip -o $InstallDir/Raef Tech-MacOS-arm64.zip -d $InstallDir
rm -f $InstallDir/Raef Tech-MacOS-arm64.zip
...
curl --head $HostName/Content/Raef Tech-MacOS-arm64.zip | grep -i "etag" | cut -d' ' -f 2 > $InstallDir/etag.txt
